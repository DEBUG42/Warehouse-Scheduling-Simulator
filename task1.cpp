...
if(updateClock2.getElapsedTime().asSeconds() >= 1.0){
	std::cout <<"vehicle0 "<<vehicles[0].position_m << std::endl;
	std::cout <<"vheicle1 "<<vehicles[1].position_m << std::endl;
	std::cout <<"vehicle2 "<<vehicles[2].position_m<< std::endl;
	std::cout<<std::endl;
...
//到车库提前减速	
	else if((vehicle->m_state.currentSpeed)*(vehicle->m_state.currentSpeed)/(2*vehicle->m_acceleration)>= fabs(distancetodevice)){
