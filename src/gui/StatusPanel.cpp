...
void StatusPanel::resize(float height)
{
    ...
    // 子组件的大小和位置会在render时或通过其他方式（如传递高度给TaskListView）调整
}

void StatusPanel::setSimulationTime(float time)
{
    std::ostringstream oss;
    oss << "Sim Time: " << std::fixed << std::setprecision(1) << time << " s";
...
void StatusPanel::setVehicleCount(size_t count)
{
    std::ostringstream oss;
    oss << "Vehicles: " << count;
    m_vehicleCountDisplay.setString(oss.str());
}

void StatusPanel::setCompletedTaskCount(size_t count)
{
    std::ostringstream oss;
    oss << "Tasks Done: " << count;
    m_completedTasksDisplay.setString(oss.str());
}

void StatusPanel::setPendingTaskCount(size_t count)
{
    std::ostringstream oss;
    oss << "Tasks Pending: " << count;
    m_pendingTasksDisplay.setString(oss.str());
}