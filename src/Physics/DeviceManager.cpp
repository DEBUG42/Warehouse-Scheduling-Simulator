...
void DeviceManager::initializeDevices()
{
    ...
    for (int id = 1; id <= 18; ++id)
    {
        ...
        {
            deviceType[id] = DeviceType::StorageIn; // 入库接口设备
        }
        else if (id == 2 || id == 4 || id == 6 || id == 8 || id == 10 || id == 12)
        {
            deviceType[id] = DeviceType::StorageOut; // 出库接口设备
        }
        else if (id >= 13 && id <= 15)
        {
            deviceType[id] = DeviceType::WorkstationOut; // 出库作业口
        }
        else if (id >= 16 && id <= 18)
        {
            deviceType[id] = DeviceType::WorkstationIn; // 入库作业口
        }
    }

    std::cout << "[DeviceManager] Initialized all 18 devices with types.\n";
}
...
std::vector<DeviceBase> DeviceManager::getAllDevices()
{
    ...
    for (const auto &[id, state] : deviceStates)
    {
        DeviceType type = deviceType[id];
        DeviceBase device(id, type);
        device.m_status = state;
...
std::vector<DeviceManager::DeviceUpdateResult> DeviceManager::update(double current_time)
{
    ...
    for (auto &[id, state] : deviceStates)
    {
        ...
        {
            results.push_back({DeviceManager::DeviceEventTrigger::ForkliftPutToInPort,
                               id,
                               -1});
        }
        ...
        {
            results.push_back({DeviceManager::DeviceEventTrigger::HumanUnloadAtOutPort,
                               id,
                               -1});
        }
        ...
        {
            results.push_back({DeviceManager::DeviceEventTrigger::StackerPickFromInInterface,
                               id,
                               -1});
        }
        ...
        {
            results.push_back({DeviceManager::DeviceEventTrigger::StackerPutToOutInterface,
                               id,
                               -1});
        }
    }
