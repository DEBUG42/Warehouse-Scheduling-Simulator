...
// 绘制轨道(包括可选的内外轨道)
void drawTrack(sf::RenderTarget &target, bool drawInnerOuterTracks = false)
{
    // 轨道颜色
    sf::Color trackColor(180, 180, 180);
    sf::Color innerTrackColor(160, 160, 160);
    sf::Color outerTrackColor(160, 160, 160);

    // 中心线绘制

    // 上直道
    sf::Vertex upperStraight[] = {
        sf::Vertex(sf::Vector2f(-STRAIGHT_LENGTH / 2 * TrackRenderer::MM_TO_PIXEL, -TRACK_RADIUS * TrackRenderer::MM_TO_PIXEL), trackColor),
        sf::Vertex(sf::Vector2f(STRAIGHT_LENGTH / 2 * TrackRenderer::MM_TO_PIXEL, -TRACK_RADIUS * TrackRenderer::MM_TO_PIXEL), trackColor)};

    // 下直道
    sf::Vertex lowerStraight[] = {
        sf::Vertex(sf::Vector2f(STRAIGHT_LENGTH / 2 * TrackRenderer::MM_TO_PIXEL, TRACK_RADIUS * TrackRenderer::MM_TO_PIXEL), trackColor),
        sf::Vertex(sf::Vector2f(-STRAIGHT_LENGTH / 2 * TrackRenderer::MM_TO_PIXEL, TRACK_RADIUS * TrackRenderer::MM_TO_PIXEL), trackColor)};

    // 绘制直道中心线
    target.draw(upperStraight, 2, sf::Lines);
    target.draw(lowerStraight, 2, sf::Lines);

    // 绘制弯道中心线
    // 右弯道
    sf::VertexArray rightCurve(sf::LineStrip);
    for (int i = 0; i <= 40; i++)
    {
        float angle = (1.5 * M_PI) + (i / 40.0f) * M_PI;
        float x = STRAIGHT_LENGTH / 2 * TrackRenderer::MM_TO_PIXEL + TRACK_RADIUS * TrackRenderer::MM_TO_PIXEL * std::cos(angle);
        float y = 0 + TRACK_RADIUS * TrackRenderer::MM_TO_PIXEL * std::sin(angle);
        rightCurve.append(sf::Vertex(sf::Vector2f(x, y), trackColor));
    }

    // 左弯道
    sf::VertexArray leftCurve(sf::LineStrip);
    for (int i = 0; i <= 40; i++)
    {
        float angle = (0.5 * M_PI) + (i / 40.0f) * M_PI;
        float x = -STRAIGHT_LENGTH / 2 * TrackRenderer::MM_TO_PIXEL + TRACK_RADIUS * TrackRenderer::MM_TO_PIXEL * std::cos(angle);
        float y = 0 + TRACK_RADIUS * TrackRenderer::MM_TO_PIXEL * std::sin(angle);
        leftCurve.append(sf::Vertex(sf::Vector2f(x, y), trackColor));
    }

    // 绘制弯道中心线
    target.draw(rightCurve);
    target.draw(leftCurve);

    // 如果需要绘制内外轨道
    if (drawInnerOuterTracks)
    {
        float halfTrackWidth = TRACK_WIDTH / 2 * TrackRenderer::MM_TO_PIXEL;

        // 内轨道绘制
        // 上直道内轨
        sf::Vertex upperInnerStraight[] = {
            sf::Vertex(sf::Vector2f(-STRAIGHT_LENGTH / 2 * TrackRenderer::MM_TO_PIXEL, (-TRACK_RADIUS - halfTrackWidth) * TrackRenderer::MM_TO_PIXEL), innerTrackColor),
            sf::Vertex(sf::Vector2f(STRAIGHT_LENGTH / 2 * TrackRenderer::MM_TO_PIXEL, (-TRACK_RADIUS - halfTrackWidth) * TrackRenderer::MM_TO_PIXEL), innerTrackColor)}; // 下直道内轨
        sf::Vertex lowerInnerStraight[] = {
            sf::Vertex(sf::Vector2f(STRAIGHT_LENGTH / 2 * TrackRenderer::MM_TO_PIXEL, (TRACK_RADIUS + halfTrackWidth) * TrackRenderer::MM_TO_PIXEL), innerTrackColor),
            sf::Vertex(sf::Vector2f(-STRAIGHT_LENGTH / 2 * TrackRenderer::MM_TO_PIXEL, (TRACK_RADIUS + halfTrackWidth) * TrackRenderer::MM_TO_PIXEL), innerTrackColor)};

        // 绘制直道内轨
        target.draw(upperInnerStraight, 2, sf::Lines);
        target.draw(lowerInnerStraight, 2, sf::Lines);

        // 绘制弯道内轨
        // 右弯道内轨
        sf::VertexArray rightInnerCurve(sf::LineStrip);
        for (int i = 0; i <= 40; i++)
        {
            float angle = (1.5 * M_PI) + (i / 40.0f) * M_PI;
            float innerRadius = (TRACK_RADIUS - TRACK_WIDTH / 2) * TrackRenderer::MM_TO_PIXEL;
            float x = STRAIGHT_LENGTH / 2 * TrackRenderer::MM_TO_PIXEL + innerRadius * std::cos(angle);
            float y = 0 + innerRadius * std::sin(angle);
            rightInnerCurve.append(sf::Vertex(sf::Vector2f(x, y), innerTrackColor));
        }

        // 左弯道内轨
        sf::VertexArray leftInnerCurve(sf::LineStrip);
        for (int i = 0; i <= 40; i++)
        {
            float angle = (0.5 * M_PI) + (i / 40.0f) * M_PI;
            float innerRadius = (TRACK_RADIUS - TRACK_WIDTH / 2) * TrackRenderer::MM_TO_PIXEL;
            float x = -STRAIGHT_LENGTH / 2 * TrackRenderer::MM_TO_PIXEL + innerRadius * std::cos(angle);
            float y = 0 + innerRadius * std::sin(angle);
            leftInnerCurve.append(sf::Vertex(sf::Vector2f(x, y), innerTrackColor));
        }

        // 绘制弯道内轨
        target.draw(rightInnerCurve);
        target.draw(leftInnerCurve);

        // 外轨道绘制
        // 上直道外轨
        sf::Vertex upperOuterStraight[] = {
            sf::Vertex(sf::Vector2f(-STRAIGHT_LENGTH / 2 * TrackRenderer::MM_TO_PIXEL, (-TRACK_RADIUS + halfTrackWidth) * TrackRenderer::MM_TO_PIXEL), outerTrackColor),
            sf::Vertex(sf::Vector2f(STRAIGHT_LENGTH / 2 * TrackRenderer::MM_TO_PIXEL, (-TRACK_RADIUS + halfTrackWidth) * TrackRenderer::MM_TO_PIXEL), outerTrackColor)};

        // 下直道外轨
        sf::Vertex lowerOuterStraight[] = {
            sf::Vertex(sf::Vector2f(STRAIGHT_LENGTH / 2 * TrackRenderer::MM_TO_PIXEL, (TRACK_RADIUS + halfTrackWidth) * TrackRenderer::MM_TO_PIXEL), outerTrackColor),
            sf::Vertex(sf::Vector2f(-STRAIGHT_LENGTH / 2 * TrackRenderer::MM_TO_PIXEL, (TRACK_RADIUS + halfTrackWidth) * TrackRenderer::MM_TO_PIXEL), outerTrackColor)};

        // 绘制直道外轨
        target.draw(upperOuterStraight, 2, sf::Lines);
        target.draw(lowerOuterStraight, 2, sf::Lines);

        // 绘制弯道外轨
        // 右弯道外轨
        sf::VertexArray rightOuterCurve(sf::LineStrip);
        for (int i = 0; i <= 40; i++)
        {
            float angle = (1.5 * M_PI) + (i / 40.0f) * M_PI;
            float outerRadius = (TRACK_RADIUS + TRACK_WIDTH / 2) * TrackRenderer::MM_TO_PIXEL;
            float x = STRAIGHT_LENGTH / 2 * TrackRenderer::MM_TO_PIXEL + outerRadius * std::cos(angle);
            float y = 0 + outerRadius * std::sin(angle);
            rightOuterCurve.append(sf::Vertex(sf::Vector2f(x, y), outerTrackColor));
        }

        // 左弯道外轨
        sf::VertexArray leftOuterCurve(sf::LineStrip);
        for (int i = 0; i <= 40; i++)
        {
            float angle = (0.5 * M_PI) + (i / 40.0f) * M_PI;
            float outerRadius = (TRACK_RADIUS + TRACK_WIDTH / 2) * TrackRenderer::MM_TO_PIXEL;
            float x = -STRAIGHT_LENGTH / 2 * TrackRenderer::MM_TO_PIXEL + outerRadius * std::cos(angle);
            float y = 0 + outerRadius * std::sin(angle);
            leftOuterCurve.append(sf::Vertex(sf::Vector2f(x, y), outerTrackColor));
        } // 绘制弯道外轨
        target.draw(rightOuterCurve);
        target.draw(leftOuterCurve);
    }
}
