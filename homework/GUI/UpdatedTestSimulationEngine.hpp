...
public:
    ...
    void initialize() {
        ...
        for (int i = 0; i < vehicleCount; ++i) {
            ...
            vehicle.motionState = Vehicle::MotionState::Cruising;
            vehicle.currentTaskId = (i % 2 == 0) ? 101 + i : -1;
    ...
    // 获取特定设备的状态
    const DeviceState& getDeviceState(int deviceId) const {
        static DeviceState emptyState; // 默认空状态，用于找不到设备时返回
...
private:
    ...
    void createDevices() {
        ...
        for (int i = 0; i < storageInCount; ++i) {
            ...
            device.processingProgress = (i % 3 == 0) ? ((float)(i % 10)) / 10.0f : 0.0f;
        ...
        for (int i = 0; i < storageOutCount; ++i) {
            ...
            device.processingProgress = (i % 4 == 0) ? ((float)(i % 10)) / 10.0f : 0.0f;
            m_devices[deviceId] = device;
        ...
        for (int i = 0; i < workstationOutCount; ++i) {
            ...
            device.queuedTaskCount = i % 2;
            device.processingProgress = (i % 2 == 0) ? ((float)(i % 10)) / 10.0f : 0.0f;
            m_devices[deviceId] = device;
