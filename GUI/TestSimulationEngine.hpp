...
public:
    ...
    void initialize()
    {
        ...
        for (int i = 0; i < vehicleCount; ++i)
        {
            ...
            vehicle.id = i;
            vehicle.trackPosition = i * (1.0f / vehicleCount);
    ...
    void update(float deltaTime)
    {
        ...
        {
            vehicle.trackPosition += vehicle.speed * scaledDelta * 0.02f;
            if (vehicle.trackPosition >= 1.0f)
            {
                vehicle.trackPosition -= 1.0f;
            }
        }
    }
    ...
    sf::Vector2f getVehiclePosition(int vehicleIndex) const
    {
        if (vehicleIndex >= 0 && vehicleIndex < static_cast<int>(m_vehicles.size()))
        {
            ...
            // 计算车辆位置
            float angle = vehicle.trackPosition * 2.0f * M_PI;
